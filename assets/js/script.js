    ...
    await axios.get(url + "es").then((data) => {
      ...
      canciones.forEach((c, i) => {
        ...
        <tr>
          ...
          <td>${c.tono}</td>
          <td>
            <button class="btn btn-secondary" onclick="prepararCancion(${i},'${c.id
          }')">Editar</button>
            <button class="btn btn-dark" onclick="eliminarCancion(${i},'${c.id
          }')">Eliminar</button>
          </td>
        </tr>
      `;
  ...
  function eliminarCancion(i, id) {
    axios.delete(url + "?id=" + id).then(() => {
      alert("Canción " + canciones[i].titulo + " de " + canciones[i].artista + ", eliminada.");
      getData();
    });
  }
